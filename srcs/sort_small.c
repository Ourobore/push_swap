...
/*   Updated: 2021/04/16 10:46:00 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	new_min_a(t_int_list **a, t_int_list **b, int len_a)
{
	t_int_list *min_a;
...
void	new_max_a(t_int_list **a, t_int_list **b, int len_a)
{
	t_int_list *min_a;
	t_int_list *max_a;
...
void	push_on_a_small(t_int_list **a, t_int_list **b, int length_a)
{
	t_int_list	*below;
	t_int_list	*above;
...
void	sort_small(t_int_list **a, t_int_list **b)
{
	...
	while (*b)
	{
		...
		if ((*b)->data < get_min_max(*a, 0)->data)
			new_min_a(a, b, length_a);
		else if ((*b)->data > get_min_max(*a, 1)->data)
			new_max_a(a, b, length_a);
		else
			push_on_a_small(a, b, length_a);
	}
